// 事件綁定和獲取分類及產品資料
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            const response = await fetch('https://shoplane2025.click/api/categories'); // 發送 API 請求
...
document.addEventListener('click', async (event) => {
    if (event.target.classList.contains('menu-link'))
        ...
        try{
            // 發送 API 請求獲取產品資料
            const response = await fetch(`https://shoplane2025.click/api/products/${catid}`); // 使用動態路由
