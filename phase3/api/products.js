...
router.post('/add', upload.single('image'), (req, res) => {
    ...
    db.query(query, [catid, name, price, description, image], (err, results) => {
        if (err) {
...
router.post('/update', upload.single('image'), (req, res) => {
    ...
    db.query(checkSql, [pid], (err, results) => {
        if (err) 
            return res.status(500).send('Database error');
        if (results.length === 0) 
...
router.post('/delete', (req, res) => {
    ...
    db.query(checksql,[pid,catid],(err,results)=>{
        if(err){
            console.error('Database error:', err);
            return res.status(500).send('Database error');  
        }
    ...
    db.query(deletesql,[pid,catid],(err)=>{
        if(err) {
            console.error('Database error:', err);
            return res.status(500).send('Database error');
        }
...
router.get('/:catid', (req,res) =>
   ...
   const { catid } = req.params;//req.params用途：提取路由中的動態參數。來源：從路由的 URL 路徑中獲取動態參數。場景：當路由中包含動態參數時，例如 /products/:pid，這個 pid 就是從 URL 中提取的產品ID。
      ...
      db.query(query,[catid],(err,results)=>{//查詢數據庫
       if(err) 
           {
